...
/** Real Firebase Auth + Firestore implementation. No in-memory auth state is authoritative. */
class FirebaseAuthRepository(
    ...
    override suspend fun completeSetup(displayName: String, username: String, avatarUrl: String?): Result<OjasUser> {
        ...
        return try {
            ...
            user.updateProfile(UserProfileChangeRequest.Builder().setDisplayName(cleanName).apply {
                if (!avatarUrl.isNullOrBlank()) setPhotoUri(android.net.Uri.parse(avatarUrl))
    ...
    override suspend fun linkWithGoogle(idToken: String, email: String?): Result<OjasUser> {
        ...
        return try {
            ...
            val profile = ensureProfile(linked); publishProfile(profile); Result.success(profile)
        } catch (e: Exception) { Log.e(TAG, "linkWithGoogle failure", e); Result.failure(e) }
    }
    ...
    override suspend fun linkWithEmailPassword(email: String, password: String): Result<OjasUser> {
        ...
        } catch (e: Exception) { Log.e(TAG, "linkWithEmailPassword failure", e); Result.failure(e) }
    }
    ...
    override suspend fun unlinkProvider(providerId: String): Result<OjasUser> {
        ...
        return try {
            ...
            val profile = ensureProfile(updated); publishProfile(profile); Result.success(profile)
        } catch (e: Exception) { Log.e(TAG, "unlinkProvider failure", e); Result.failure(e) }
    }

    override suspend fun logout() {
        try { auth.signOut(); _authState.value = AuthState.Unauthenticated; Log.d(TAG, "logout success") }
        catch (e: Exception) { Log.e(TAG, "logout failure", e); _authState.value = AuthState.InitializationFailed(e.message ?: "Logout failed.") }
    }

    // Phone auth is deliberately disabled for OJAS; these methods never call Firebase phone auth.
    override suspend fun sendPhoneOtp(phoneNumber: String, activity: android.app.Activity?, onCodeSent: (String, Any?) -> Unit, onVerificationFailed: (Exception) -> Unit, onAutoVerified: ((OjasUser) -> Unit)?) {
        val error = UnsupportedOperationException("Phone number authentication is disabled in OJAS.")
        Log.e(TAG, "sendPhoneOtp called while disabled", error); onVerificationFailed(error)
    }
    ...
    private suspend fun ensureProfile(user: FirebaseUser): OjasUser {
        ...
        if (snapshot.exists()) {
            return mapToUser(snapshot.data.orEmpty()).copy(email = user.email, phoneNumber = user.phoneNumber, isEmailVerified = user.isEmailVerified)
        }
        val now = System.currentTimeMillis()
        val profile = OjasUser(userId = user.uid, displayName = user.displayName.orEmpty(), avatarUrl = user.photoUrl?.toString(), createdAt = user.metadata?.creationTimestamp ?: now, updatedAt = now, isSetupComplete = false, email = user.email, phoneNumber = user.phoneNumber, isEmailVerified = user.isEmailVerified)
        ref.set(userToMap(profile)).awaitLogged("createUserProfile")
        Log.d(TAG, "created Firestore profile uid=${user.uid}")
        return profile
    }
    ...
    private fun userToMap(user: OjasUser): HashMap<String, Any?> = hashMapOf(
        ...
        "email" to user.email, "phoneNumber" to user.phoneNumber, "isEmailVerified" to user.isEmailVerified
    )

    private fun mapToUser(data: Map<String, Any?>): OjasUser = OjasUser(
        userId = data["userId"] as? String ?: "", username = data["username"] as? String ?: "",
        displayName = data["displayName"] as? String ?: "", bio = data["bio"] as? String ?: "",
        avatarUrl = data["avatarUrl"] as? String, createdAt = (data["createdAt"] as? Number)?.toLong() ?: System.currentTimeMillis(),
        updatedAt = (data["updatedAt"] as? Number)?.toLong() ?: System.currentTimeMillis(),
        isSetupComplete = data["isSetupComplete"] as? Boolean ?: false, email = data["email"] as? String,
        phoneNumber = data["phoneNumber"] as? String, isEmailVerified = data["isEmailVerified"] as? Boolean ?: false
    )

    private fun <T> fail(message: String, cause: Throwable? = null): Result<T> {
        val exception = IllegalStateException(message, cause); Log.e(TAG, message, exception); return Result.failure(exception)
    }

    private suspend fun <T> Task<T>.awaitLogged(operation: String): T = suspendCancellableCoroutine { continuation ->
        addOnCompleteListener { task -> if (task.isSuccessful) Log.d(TAG, "$operation onComplete success") else Log.e(TAG, "$operation onComplete failure", task.exception) }
        addOnSuccessListener { result -> Log.d(TAG, "$operation onSuccess"); if (continuation.isActive) continuation.resume(result) }
        addOnFailureListener { error -> Log.e(TAG, "$operation onFailure", error); if (continuation.isActive) continuation.resumeWithException(error) }
        addOnCanceledListener { Log.e(TAG, "$operation cancelled"); continuation.cancel() }
    }
}
